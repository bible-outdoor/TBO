...
// === Robust Token Checks and Error Handling ===
...
async function fetchWithAuth(url, options = {}) {
  const token = getAdminTokenSafe();
  if (!token) {
    window.location.href = 'login.html?msg=session_expired';
    throw new Error('No admin token found. Redirecting to login.');
  }
...
// --- Replace direct fetch calls with fetchWithAuth for all backend API requests below ---
// Example for fetchUsers:
async function fetchUsers() {
  return await fetchWithAuth('https://bible-outdoor-backend.onrender.com/api/users');
}
...
async function updateUser(email, user) {
  return await fetchWithAuth(`https://bible-outdoor-backend.onrender.com/api/users/${encodeURIComponent(email)}`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(user)
  });
...
function getLastActivity() {
  return parseInt(localStorage.getItem('adminLastActivity') || '0', 10);
}

function checkSessionExpiry() {
  const token = getAdminToken && typeof getAdminToken === 'function' ? getAdminToken() : localStorage.getItem('adminToken');
  if (!token) return; // No session, nothing to check
  const last = getLastActivity();
  if (last && Date.now() - last > SESSION_TIMEOUT_MS) {
    clearAdminSession && typeof clearAdminSession === 'function' ? clearAdminSession() : (localStorage.removeItem('adminToken'), localStorage.removeItem('adminUser'));
...
['click','keydown','mousemove','scroll','touchstart'].forEach(evt => {
  window.addEventListener(evt, setLastActivity, true);
});
